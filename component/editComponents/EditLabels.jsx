...
import { addLabel, DisplayAllLabel, editLabel } from "../service/Services";
import EditShowWordDelete from './EditShowWordDelete'
...
class EditLabels extends React.Component {
  ...
  handleeditdata = label => {this.setState({labelid: label});};

  handleeditlabel = event => { this.setState({ editlabel: event.target.value });
    console.log("ddd" + this.state.editlabel);
  };

  handleClickOpen = () => { this.setState({ open: true }); };

  Handlelabeldata = event => { this.setState({ createLabel: event.target.value }); }; 

  handleClose = () => { this.setState({ open: false });

    let token = localStorage.getItem("token"); 

    editLabel(this.state.editlabel, this.state.labelid, token) .then(Response => 
      {
        console.log("label edit sucess");
        this.props.refresh()
      })
      .catch(err => {console.log("label not edit sucess"); });
  };

  UNSAFE_componentWillMount() { this.getAllLabel(); }
  ...
  handleClickCreatelabel = () => 
  {
    ...
    addLabel(data, token)
      ...
      .then(Response => {
        ...
        console.log(Response,"label created..");
        this.props.refresh()
        this.getAllLabel();
      }).catch(err => { console.log("label not created.."); });
  };
              ...
              <div>
                ...
                <div style={{ display: "flex", justifyContent: "space-around", paddingTop: "5%"}} >
                    <span>
                      ...
                      <EditShowWordDelete deleteidprops={label.id}  refresh={this.getAllLabel}/>
